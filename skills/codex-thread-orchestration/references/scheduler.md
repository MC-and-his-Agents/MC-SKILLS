...
scheduler 默认不要创建长期 active goal。除非用户明确要求 scheduler goal，或调度动作短且能闭环，否则使用 heartbeat 和 `orchestration_state_root/state/dispatch-table.json` 保持活性。

## Dispatch Table / 调度表

维护 scheduler-owned table。完整表写入 `orchestration_state_root/state/dispatch-table.json`，不要塞进 scheduler thread、heartbeat prompt 或跨线程正文：
...
每次 worker report artifact、hosted check readback、gate result、merge、blocker triage 和 closeout event 后都要更新 state file。消费 worker report 时必须登记 `report_consumed`，包括 `report_id`、`report_path`、`report_state`、`consumed_at`、`state_file_updated` 和 `next_owner`。

事实冲突优先级：live host/local git readback > repo carrier current files > orchestration carrier state/consumption records > newest worker report artifact > current live locator notice > older heartbeat summary。旧 heartbeat、thread preview 和未消费正文不得覆盖 newer readback。
...
- startup report artifact 写入 `report_output_path`，且 locator notice 能回到 scheduler thread。
...
- `read_thread`：只允许对当前 live thread 做短 locator/ACK 可达性确认；不要读取 retired/systemError/abandoned thread turns，不要用 thread preview 重建状态，不要当作 goal API。
...
- 如果 `send_message_to_thread` 不可用，使用 `reporting.md` 的 locator-only fallback：要求 worker 先写 `report_output_path`，只输出 `Scheduler Report Notice`。没有可写 report path 时，标记 `report-path-missing` blocker。
- 如果 `read_thread` 不可用，不能用旧线程正文恢复；在改变 scheduler state 前，必须从 `report_path`、项目事实源或当前可达 locator notice 取得证据。
- 如果 heartbeat automation 不可用，把 compact heartbeat prompt 当作 scheduler-local checklist，并回报 liveness limitation；不要为了轮询而创建长期 scheduler goal，也不要把完整 dispatch table 粘进 prompt。
...
scheduler 发送后在 `dispatch-table.json` 标记 `instruction-sent-awaiting-ack`。worker 的 `instruction_ack` locator 或 startup/recovery report artifact 被消费前，不得把 worker 标为 `active`，不得声称该指令已被 worker 理解或执行。下一次 heartbeat 仍无 ACK locator 且项目事实无变化时，必须重发、纠正路由、创建 replacement，或标记为 classified tool/global blocker。
...
report_output_path: <state_root>/reports/<report_id>.json
...
scheduler 读取 `report_path` 并消费 worker report 后，必须在 `orchestration_state_root/consumption/` 和 dispatch table 记录：
...
没有 `report_consumed` 时，不得把 worker report 视为已经驱动了 table transition。若 report artifact 缺 `report_id`、`report_path` 或 `report_for_instruction_id`，scheduler 可以用项目事实源处理紧急事实，但必须标记 `report_locator_missing` 并在下一条 correction 中要求补齐。
...
- `valid_wait`：等待对象仍有效，并记录具体 report locator、hosted run locator、PR/head、项目事实源 readback、等待原因和 `next_decision_at`。合法等待只能由 active worker 最近 report locator、同一 hosted run 仍在运行、外部锁/权限/队列有界等待等证明；不得用 thread preview 或 worker turn 正文证明。
...
- 把旧 heartbeat summary 当作最新事实，覆盖新的 worker report artifact、host readback 或 repo carrier。
...
- 合法等待对象必须是 external、带 recent report locator 的 active worker、hosted run、另一个 owner 或有界外部锁；不得把当前 scheduler 自己的待办写成 valid_wait。
...
- 当前 live worker 只有 metadata 可达但没有产生 expected report locator，且 recovery index 显示已超过本轮 deadline。
- scheduler 已发送 recovery/checkpoint prompt，但下一次 heartbeat 仍没有 scheduler-readable report locator。
...
- `dispatch-table.json` 的 `next_scheduler_action` 必须改为 `create replacement worker` 或 `scheduler controlled takeover`。
- heartbeat prompt 必须从“继续读该 worker 状态”改成跟踪 replacement/recovery locator path。
...
下一次 heartbeat 如果没有收到 expected report locator，或 PR/worktree/base 事实没有变化，必须升级为 `worker-stalled`。只有看到新的 report artifact 被消费，或 host/worktree 事实变化，才能继续等待。
...
- replacement worker objective 必须包含 stalled worker id/thread、recovery reason、exact branch/worksite/base/head、allowed write paths、forbidden scope、validation requirements、`gate_owner=scheduler`、`report_to_thread_id=scheduler_thread_id`、`orchestration_state_root`、`report_output_path`、`report_size_budget` 和 `do_not_read_retired_thread_turns: true`。
- replacement worker 不接收旧线程历史，不读取 abandoned/retired/systemError thread turns。恢复只能来自 `orchestration_state_root`、项目事实源、current branch/worksite readback 和明确 report locator。
