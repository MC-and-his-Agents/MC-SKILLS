...
它读取 `orchestration_state_root` 下的 state/report locator，不读取线程历史作为状态数据库。
...
- scheduler report artifact 是否已被 watcher 消费并更新 pool。
- lane request / release / scheduler blocked update 是否已被 watcher 消费。
- `state/lane-lock-table.json`、`state/waiting-queue.json` 和 release predicates 是否最新。
...
- schedule。
- status。
- prompt 中的 `orchestration_state_root`、待消费 locator、budget 和 forbidden actions 是否最新。
...
如果 live facts、scheduler report locators、lane table、waiting queue 和 release predicates 都没有变化，heartbeat 应输出 `DONT_NOTIFY`，不要打扰用户。提高 cadence 只表示需要更快读回 lane/recovery 状态，不表示可以扩容 scheduler pool。
...
- 向 scheduler 发送误发 worker report 的 locator-only notice。
- 标记 scheduler complete/stalled/retired。
- 切换 unit cursor。

有效 `valid_wait` 必须证明至少一个 scheduler 仍在有效推进，或 watcher 正在等待明确外部事实、lane owner、hosted run、release predicate readback。旧 watcher summary、未 ACK 的 scheduler instruction、未消费的 scheduler report locator、未消费的 lane request、不可读 scheduler、无 heartbeat target readback 都不是合法等待对象。
...
- 是否存在比 watcher prompt 更新的 scheduler report locator。
...
如果 scheduler report 或 lane-level report locator 未消费，必须先读取 `report_path`、记录对应 consumed event，并更新 scheduler pool、lane_lock_table 或 waiting_queue，再判断 unit state、parallel decision、lane grant/release 或 replacement。旧 watcher prompt 不得覆盖更新的 scheduler/lane report artifact。
...
3. 只向 scheduler 发送 worker report locator notice。
4. 要求 scheduler 修正 worker `report_to_thread_id`。
5. watcher 只记录 routing incident，不更新 unit completion。
6. 如果没有 `report_path`，记录 `misrouted-worker-report-path-missing` blocker，不把完整 report 塞进线程。

## Stale Prompt Rules / 过期提示规则

每次生成 watcher prompt 前检查：

- scheduler pool 是否有新 scheduler、新 heartbeat 或 retired/systemError scheduler。
...
发现 prompt stale 时先更新 automation，再继续判断。不要让旧 prompt 持续创建旧 scheduler 或等待已完成 unit。更新 prompt 时只放 `orchestration_state_root`、locator、checklist、hard forbidden actions 和 budget。
